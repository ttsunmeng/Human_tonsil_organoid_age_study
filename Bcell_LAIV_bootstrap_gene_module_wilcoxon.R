...
cor_cluster_table <- readRDS('Bcell_LAIV_module_cor_cluster_table.rds')
Bcell_module_dataframe <- readRDS('Bcell_LAIV_gene_module_table.rds')
...
age_group_list <- sort(unique(Bcell_module_dataframe$age_group))
...
condition_list <- unique(Bcell_module_dataframe$condition)
...
Bcells_diff_wilcoxon <- foreach (index = 1:n_bootstrap,.combine = "rbind") %dopar% {#,.options.snow = opts
  ...
  for (condition_name in condition_list){
    temp <- which(Bcell_module_dataframe$condition == condition_name)
  ...
  for (module_name in module_list) {
    ...
    if (length(min_feature_list) > 0){
      temp_lognorm_gene_data_all$min_expr <- Bcell_module_dataframe[,paste('min_',module_name,sep = '')][temp_lognorm_gene_data_all$index]
      temp_output$n_min_feature[row_select] <- length(min_feature_list)
    } else {
      temp_output$n_min_feature[row_select] <- 0
    }
    temp_lognorm_gene_data_all$expr <- Bcell_module_dataframe[,module_name][temp_lognorm_gene_data_all$index]
    temp_lognorm_gene_data_all$age_group <- Bcell_module_dataframe$age_group[temp_lognorm_gene_data_all$index]
    temp_lognorm_gene_data_all$day_group <- Bcell_module_dataframe$day_group[temp_lognorm_gene_data_all$index]
...
saveRDS(Bcells_diff_wilcoxon,'tonsil_LAIV_120a_s_Bcell_LAIV_lognorm_scale_module_age_wilcoxon_output_parallel_n300.rds')
