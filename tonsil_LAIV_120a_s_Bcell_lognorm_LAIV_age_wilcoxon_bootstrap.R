...
Bcell_LAIV_wilcoxon <- readRDS('Bcell_age_LAIV_wilcoxon_bootstrap/tonsil_LAIV_120a_s_preGCOrGCB_LAIV_lognorm_age_wilcoxon_output_parallel_nmean.rds')
############# examine output overlaps #######################################
Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon[!is.na(Bcell_LAIV_wilcoxon$pval),]
Bcell_LAIV_wilcoxon$log2FC <- log2(Bcell_LAIV_wilcoxon$mean1/Bcell_LAIV_wilcoxon$mean2)
Bcell_LAIV_wilcoxon$log2FC[Bcell_LAIV_wilcoxon$log2FC %in% c(Inf,-Inf,NaN)] <-
  log2(Bcell_LAIV_wilcoxon$mean1[Bcell_LAIV_wilcoxon$log2FC %in% c(Inf,-Inf,NaN)] + 1)
all_marker_table <- read.xlsx('tonsil_LAIV_120a_s_background_list.xlsx', sheetName = "Sheet1")
p <- match(Bcell_LAIV_wilcoxon$marker, all_marker_table$marker)
temp <- all_marker_table[p,]
Bcell_LAIV_wilcoxon$ENTREZID <- temp$ENTREZID
Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon %>% group_by(day_group,age_group1,age_group2) %>% 
  mutate(pval_adj = p.adjust(pval,method = 'fdr'))


Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon %>% group_by(day_group,marker,age_group1,age_group2) %>% 
  mutate(pval_adj_mean = exp(mean(log(pval_adj))),log2FC_mean = mean(log2FC),pct1_mean = mean(pct1),pct2_mean = mean(pct2))

Bcell_LAIV_wilcoxon$effect <- '+'
Bcell_LAIV_wilcoxon$effect[Bcell_LAIV_wilcoxon$log2FC < 0] <- '-'
Bcell_LAIV_wilcoxon$significant <- 'no'
Bcell_LAIV_wilcoxon$significant[(Bcell_LAIV_wilcoxon$pval_adj <= p_cutoff)] <- 'yes'

Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon %>% group_by(day_group,marker,age_group1,age_group2,effect) %>% 
  mutate(n_significant = sum(significant == 'yes'))
n_bootstrap_final <- max(Bcell_LAIV_wilcoxon$bootstrap_index)
Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon %>% group_by(day_group,marker,age_group1,age_group2,effect) %>% 
  mutate(significant = ifelse((n_significant > floor(n_bootstrap_final*0.95)), "yes", "no"))#
Bcell_LAIV_wilcoxon$label <- Bcell_LAIV_wilcoxon$marker
Bcell_LAIV_wilcoxon$label[Bcell_LAIV_wilcoxon$significant == 'no'] <- ''
Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon %>% group_by(day_group,marker,age_group1,age_group2) %>% 
  mutate(effect = ifelse(log2FC_mean > 0, "+", "-"))#

Bcell_LAIV_wilcoxon <- Bcell_LAIV_wilcoxon[!Bcell_LAIV_wilcoxon$day_group %in% c("day14",'day07'),]
LAIV_day_group_list <- as.character(unique(Bcell_LAIV_wilcoxon$day_group))
# LAIV_day_group_list_youth <- LAIV_day_group_list#LAIV_day_group_list[!LAIV_day_group_list %in% c("day14",'day07')]

Bcell_LAIV_wilcoxon_youth <- Bcell_LAIV_wilcoxon[(Bcell_LAIV_wilcoxon$age_group1 == '02-03yrs'),]
Bcell_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth[(Bcell_LAIV_wilcoxon_youth$age_group2 == '27-39yrs'),]
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays <- Bcell_LAIV_wilcoxon_youth_vs_adult
Bcell_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult[Bcell_LAIV_wilcoxon_youth_vs_adult$day_group %in% LAIV_day_group_list,]

# temp <- Bcell_LAIV_wilcoxon_youth_vs_adult[Bcell_LAIV_wilcoxon_youth_vs_adult$marker == 'CCR10',]

Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult[(Bcell_LAIV_wilcoxon_youth_vs_adult$significant == 'yes') & 
                                            (Bcell_LAIV_wilcoxon_youth_vs_adult$effect == '+'),]
Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation <-
  unique(Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','pct1_mean','pct2_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation <- Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation %>% arrange(days,desc(log2FC_mean)) 
Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation <- Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.5))
# Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation <- 
#   Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation[abs(Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation$log2FC_mean) > 0.25,]
# 
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation),sheetName = 'upregulation',
           'tonsil_LAIV_120a_s_Bcells_LAIV_lognorm_wilcoxon_bootstrap_n300_toddler_vs_adult.xlsx')
# unique(Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation[,c('day_group','n_significant','n_distinct')])


Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult[(Bcell_LAIV_wilcoxon_youth_vs_adult$significant == 'yes') & 
                                            (Bcell_LAIV_wilcoxon_youth_vs_adult$effect == '-'),]
Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation <-
  unique(Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','pct1_mean','pct2_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation %>% arrange(days,log2FC_mean) 
Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.5))
# Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation <- 
#   Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation[abs(Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$log2FC_mean) > 0.5,]
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation),sheetName = 'downregulation',
           'tonsil_LAIV_120a_s_Bcells_LAIV_lognorm_wilcoxon_bootstrap_n300_toddler_vs_adult.xlsx',append = T)

# temp1 <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation[Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group != 'day0',]
# temp1 <- temp1[temp1$log2FC_mean < -1,]
# temp2 <- Bcell_memory_LAIV_wilcoxon_youth_vs_adult_downregulation$marker[Bcell_memory_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group == 'day0']
# # 
# # day_group_name <- 'day14'
# # temp1 <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$marker[Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group == day_group_name]
# # temp2 <- Bcell_memory_LAIV_wilcoxon_youth_vs_adult_downregulation$marker[Bcell_memory_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group == day_group_name]
...
#   graphics.off()
#   temp <- unique(Bcell_LAIV_wilcoxon_youth_vs_adult[(Bcell_LAIV_wilcoxon_youth_vs_adult$day_group == day_group_name),][,c('marker','log2FC_mean','pval_adj_mean','effect','significant','label')])
...
#   print(plot)
#   dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_bootstrap_toddler_vs_adult_',day_group_name,'.pdf',sep = ''),width = 6.5, height = 5)
# }

# visualization of all day groups together
temp <- Bcell_LAIV_wilcoxon_youth_vs_adult[!Bcell_LAIV_wilcoxon_youth_vs_adult$day_group %in% c('day07-08'),]
temp$log2FC_mean <- -temp$log2FC_mean
...
ggplot(DEG_count, aes(fill=sig_sign, y=count, x=day_group)) + 
  ...
  ylab('#DEGs of Bcells') + RotatedAxis()
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_lognorm_bootstrap_n300_toddler_vs_adult_numDEGs.pdf',sep = ''),width = 3.5, height = 3)
...
temp$label[temp$label %in% c('ab-CD3','ab-Bcell','ab-TCR-gamma-delta','ab-CD8','ab-TCR-alpha-beta')] <- ''
...
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n300_toddler_vs_adult_2.pdf',sep = ''),width = 6.5, height = 5)


temp <- Bcell_LAIV_wilcoxon_youth_vs_adult
temp$log2FC_mean <- -temp$log2FC_mean
temp <- unique(temp[,c('day_group','marker','log2FC_mean','pval_adj_mean','effect','significant','label')])
temp$significant[(abs(temp$log2FC_mean) < 0.25) & (temp$significant == 'yes')] <- 'no'
...
ggplot(temp, aes(x=log2FC_mean, y=-log10(pval_adj_mean),color = significant)) +
  ...
  geom_vline(xintercept=c(-log2FC_cutoff,log2FC_cutoff), linetype="dotted") +
  geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n300_toddler_vs_adult_day0.pdf',sep = ''),width = 6.5, height = 5)
...
library('xlsx')
Bcell_lognorm_DEG_upreg <- read.xlsx('Bcell_age_LAIV_wilcoxon_bootstrap/tonsil_LAIV_120a_s_Bcell_LAIV_lognorm_wilcoxon_bootstrap_n300_toddler_vs_adult.xlsx',sheetName = 'downregulation')
Bcell_lognorm_DEG_downreg <- read.xlsx('Bcell_age_LAIV_wilcoxon_bootstrap/tonsil_LAIV_120a_s_Bcell_LAIV_lognorm_wilcoxon_bootstrap_n300_toddler_vs_adult.xlsx',sheetName = 'upregulation')
Bcell_lognorm_DEG_upreg_day0 <- Bcell_lognorm_DEG_upreg[Bcell_lognorm_DEG_upreg$day_group == 'day0',]
Bcell_lognorm_DEG_upreg_nonday0 <- Bcell_lognorm_DEG_upreg[Bcell_lognorm_DEG_upreg$day_group != 'day0',]
Bcell_lognorm_DEG_downreg_day0 <- Bcell_lognorm_DEG_downreg[Bcell_lognorm_DEG_downreg$day_group == 'day0',]
Bcell_lognorm_DEG_downreg_nonday0 <- Bcell_lognorm_DEG_downreg[Bcell_lognorm_DEG_downreg$day_group != 'day0',]

# devtools::install_github("yanlinlin82/ggvenn")
library(ggvenn)

x <- list(
  up_day0 = unique(Bcell_lognorm_DEG_upreg_day0$marker), 
  up_later = unique(Bcell_lognorm_DEG_upreg_nonday0$marker), 
  down_later = unique(Bcell_lognorm_DEG_downreg_nonday0$marker),
  down_day0 = unique(Bcell_lognorm_DEG_downreg_day0$marker)
)
ggvenn(
  x, 
...
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_lognorm_age_DEG_venn_up&down.pdf',width = 5, height = 5)
...
x <- list(
  up_day0 = unique(Bcell_lognorm_DEG_upreg_day0$marker), 
  up_day4 = unique(Bcell_lognorm_DEG_upreg_nonday0$marker[Bcell_lognorm_DEG_upreg_nonday0$day_group == 'day04']), 
  up_day7 = unique(Bcell_lognorm_DEG_upreg_nonday0$marker[Bcell_lognorm_DEG_upreg_nonday0$day_group == 'day06-07']), 
  up_day10 = unique(Bcell_lognorm_DEG_upreg_nonday0$marker[Bcell_lognorm_DEG_upreg_nonday0$day_group == 'day10']),
  up_day14 = unique(Bcell_lognorm_DEG_upreg_nonday0$marker[Bcell_lognorm_DEG_upreg_nonday0$day_group == 'day12-14'])
)
ggVennDiagram(
  x, 
...
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_lognorm_age_DEG_upregulation_venn_days.pdf',width = 5, height = 5)

ggVennDiagram(x,force_upset = TRUE,order.set.by = "none",order.intersect.by = "none")
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_lognorm_age_DEG_upregulation_bars_intersect_days.pdf',width = 5, height = 4)

x <- list(
  down_day0 = unique(Bcell_lognorm_DEG_downreg_day0$marker), 
  down_day4 = unique(Bcell_lognorm_DEG_downreg_nonday0$marker[Bcell_lognorm_DEG_downreg_nonday0$day_group == 'day04']), 
  down_day7 = unique(Bcell_lognorm_DEG_downreg_nonday0$marker[Bcell_lognorm_DEG_downreg_nonday0$day_group == 'day06-07']), 
  down_day10 = unique(Bcell_lognorm_DEG_downreg_nonday0$marker[Bcell_lognorm_DEG_downreg_nonday0$day_group == 'day10']),
  down_day14 = unique(Bcell_lognorm_DEG_downreg_nonday0$marker[Bcell_lognorm_DEG_downreg_nonday0$day_group == 'day12-14'])
)
ggVennDiagram(
  x, 
  fill_color = c("#868686FF","#0073C2FF", "#EFC000FF", "#CD534CFF",'#BAE4B3'),
...
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_lognorm_age_DEG_downregulation_venn_days.pdf',width = 5, height = 5)

ggVennDiagram(x,force_upset = TRUE,order.set.by = "none",order.intersect.by = "none")
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_lognorm_age_DEG_downregulation_bars_intersect_days.pdf',width = 5, height = 4)
...
ggplot(temp, aes(x=log2FC_mean, y=-log10(pval_adj_mean),color = significant)) +
  ...
  geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
# dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n300_toddler_vs_adult_big.pdf',sep = ''),width = 15, height = 15)
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n300_toddler_vs_adult_cytokine_only.pdf',sep = ''),width = 10, height = 10)

all_Bcell_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
NaiveOrActB_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
PB_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
GClikeB_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
MemoryB_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
preGCOrGCB_LAIV_wilcoxon_youth_vs_adult <- Bcell_LAIV_wilcoxon_youth_vs_adult
...
HM2 <- draw(HM2)
dev.print(pdf, 'tonsil_LAIV_120a_s_Bcell_LAIV_cytokine_DEG_log2FC.pdf',width = 6, height = 4)
...
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays[(Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$effect == '+'),] 
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays[(Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$effect == '-'),] 

Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation %>% group_by(marker) %>% 
...
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation %>% group_by(marker) %>% 
  summarize(log2FCmax = max(-log2FC_mean),day_group = day_group[which.max(-log2FC_mean)],if_significant = significant[which.max(-log2FC_mean)])

Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation_sig <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays[(Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$significant == 'yes') & 
                                               (Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$effect == '+'),] 
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation_sig <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays[(Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$significant == 'yes') & 
                                               (Bcell_LAIV_wilcoxon_youth_vs_adult_alldays$effect == '-'),] 
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation_sig <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation_sig %>% group_by(marker) %>% 
  summarize(log2FCmax = -max(log2FC_mean),day_group = day_group[which.max(log2FC_mean)])
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation_sig <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation_sig %>% group_by(marker) %>% 
  summarize(log2FCmax = max(-log2FC_mean),day_group = day_group[which.max(-log2FC_mean)])

Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation$celltype <- 'BCL6-nonmemory B'
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation$celltype <- 'BCL6-nonmemory B'
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation_sig$celltype <- 'BCL6-nonmemory B'
Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation_sig$celltype <- 'BCL6-nonmemory B'

Bcell_subset_DEG_downregulation <- rbind(Bcell_subset_DEG_downregulation,Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation)
Bcell_subset_DEG_upregulation <- rbind(Bcell_subset_DEG_upregulation,Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation)
Bcell_subset_DEG_downregulation_sig <- rbind(Bcell_subset_DEG_downregulation_sig,Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_upregulation_sig)
Bcell_subset_DEG_upregulation_sig <- rbind(Bcell_subset_DEG_upregulation_sig,Bcell_LAIV_wilcoxon_youth_vs_adult_alldays_downregulation_sig)
...
for (day_group_name in LAIV_day_group_list){
  print(day_group_name)
  temp <- Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation[Bcell_LAIV_wilcoxon_youth_vs_adult_upregulation$day_group == day_group_name,]
  ...
  if (dim(ego)[1] != 0) {
    ...
    write.xlsx(ego@result,paste('tonsil_LAIV_120a_s_lognorm_Bcell_preGCOrGCB_LAIV_wilcoxon_bootstrap_nmean_toddler_vs_adult_GO_ClusterProfiler.xlsx',sep = ''),
               sheetName = paste(day_group_name,'upregulation'),append = T)
  }
  temp <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation[Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$day_group == day_group_name,]
  ...
  if (dim(ego)[1] != 0) {
    ...
    print(plot)
    dev.print(pdf, paste('tonsil_LAIV_120a_s_lognorm_Bcell_LAIV_preGCOrGCB_wilcoxon_bootstrap_nmean_toddler_vs_adult_GO_ClusterProfiler_downregulation_',day_group_name,'.pdf',sep = ''),width = 8, height = 10)
  }
}
...
print(plot)
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n316_toddler_vs_adult_GO_ClusterProfiler_upregulation_day07-08.pdf',sep = ''),
          width = 10, height = 10)
# simplify
...
table1 <- 'tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_toddler_vs_adult_downregulation_day04-06_DAVID_UP_KW_BIOLOGICAL_PROCESS.txt'
table1_DE_data <- read.table(table1, sep = '\t',header = T)
day_group_name <- 'day04-06'
temp <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation %>% dplyr::filter(day_group == day_group_name)
...
# theme(axis.text = element_text(size = 20),plot.title = element_text(size = 25, face = "bold"))
dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_toddler_vs_adult_downregulation_day04-06_UP_KW_BP.pdf',sep = ''),width = 4.5, height = 2.5)
...
age_group1 <- '02-03yrs'
age_group2 <- '07-09yrs'
temp <- Bcell_LAIV_wilcoxon[(Bcell_LAIV_wilcoxon$age_group1 == age_group1),]
...
#     geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
#   print(plot)
#   dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_toddlers_vs_tweens_',day_group_name,'.pdf',sep = ''),width = 6.5, height = 5)
# }
...
#     geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
#   print(plot)
#   dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_tweens_vs_adults_',day_group_name,'.pdf',sep = ''),width = 6.5, height = 5)
# }
############# stringent in toddler vs both ########################
Bcell_LAIV_wilcoxon_youth_stringent <- Bcell_LAIV_wilcoxon_youth %>% group_by(day_group,marker) %>% 
  mutate(significant = ifelse(all(significant == 'yes') & (prod(log2FC_mean) > 0),'yes','no'))
        
        
# temp <- unique(Bcell_LAIV_wilcoxon_youth_stringent[,c('day_group','marker','age_group1','age_group2','significant','significant2','log2FC_mean','effect')])
# temp <- temp %>% arrange(day_group,marker)

Bcell_LAIV_wilcoxon_youth_stringent <- Bcell_LAIV_wilcoxon_youth_stringent %>% group_by(day_group,marker) %>% 
  mutate(pval_adj_mean = exp(mean(log(unique(pval_adj_mean)))),log2FC_mean = mean(unique(log2FC_mean)),n2 = mean(unique(n2)))
Bcell_LAIV_wilcoxon_youth_stringent <- Bcell_LAIV_wilcoxon_youth_stringent %>% group_by(day_group,marker) %>% 
  mutate(effect = ifelse(log2FC_mean > 0, "+", "-"))
Bcell_LAIV_wilcoxon_youth_stringent$label[Bcell_LAIV_wilcoxon_youth_stringent$significant == 'no'] <- ''

Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- 
  Bcell_LAIV_wilcoxon_youth_stringent[(Bcell_LAIV_wilcoxon_youth_stringent$significant == 'yes') & 
                                       (Bcell_LAIV_wilcoxon_youth_stringent$effect == '+'),]
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <-
  unique(Bcell_LAIV_wilcoxon_youth_stringent_upregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_stringent_upregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_youth_stringent_upregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- Bcell_LAIV_wilcoxon_youth_stringent_upregulation %>% arrange(days,desc(log2FC_mean)) 
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- Bcell_LAIV_wilcoxon_youth_stringent_upregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.25))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_stringent_upregulation),sheetName = 'upregulation',
           'tonsil_LAIV_120a_sBcell_lognorm_LAIV_bootstrap_stringent_toddler_vs_both.xlsx')

Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- 
  Bcell_LAIV_wilcoxon_youth_stringent[(Bcell_LAIV_wilcoxon_youth_stringent$significant == 'yes') & 
                                       (Bcell_LAIV_wilcoxon_youth_stringent$effect == '-'),]
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <-
  unique(Bcell_LAIV_wilcoxon_youth_stringent_downregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_stringent_downregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_youth_stringent_downregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- Bcell_LAIV_wilcoxon_youth_stringent_downregulation %>% arrange(days,log2FC_mean) 
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- Bcell_LAIV_wilcoxon_youth_stringent_downregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.25))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_stringent_downregulation),sheetName = 'downregulation',
           'tonsil_LAIV_120a_sBcell_lognorm_LAIV_bootstrap_stringent_toddler_vs_both.xlsx',append = T)

# for (day_group_name in LAIV_day_group_list) {
#   print(day_group_name)
#   graphics.off()
#   temp <- unique(Bcell_LAIV_wilcoxon_youth_stringent[(Bcell_LAIV_wilcoxon_youth_stringent$day_group == day_group_name),][,c('marker','log2FC_mean','pval_adj_mean','effect','significant','label')])
...
#     geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
#   print(plot)
#   dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_stringent_toddler_vs_both_',day_group_name,'.pdf',sep = ''),width = 6.5, height = 5)
# }

############# stringent in adult vs both ########################
Bcell_LAIV_wilcoxon_adult <- Bcell_LAIV_wilcoxon[(Bcell_LAIV_wilcoxon$age_group2 == '27-39yrs'),]
Bcell_LAIV_wilcoxon_adult_stringent <- Bcell_LAIV_wilcoxon_adult %>% group_by(day_group,marker) %>% 
  mutate(significant = ifelse(all(significant == 'yes') & (prod(unique(log2FC_mean)) > 0),'yes','no'))
Bcell_LAIV_wilcoxon_adult_stringent <- Bcell_LAIV_wilcoxon_adult_stringent %>% group_by(day_group,marker) %>% 
  mutate(pval_adj_mean = exp(mean(log(unique(pval_adj_mean)))),log2FC_mean = mean(unique(log2FC_mean)),n1 = mean(unique(n1)))
Bcell_LAIV_wilcoxon_adult_stringent <- Bcell_LAIV_wilcoxon_adult_stringent %>% group_by(day_group,marker) %>% 
  mutate(effect = ifelse(log2FC_mean > 0, "+", "-"))
Bcell_LAIV_wilcoxon_adult_stringent$label[Bcell_LAIV_wilcoxon_adult_stringent$significant == 'no'] <- ''

Bcell_LAIV_wilcoxon_adult_stringent_upregulation <- 
  Bcell_LAIV_wilcoxon_adult_stringent[(Bcell_LAIV_wilcoxon_adult_stringent$significant == 'yes') & 
                                        (Bcell_LAIV_wilcoxon_adult_stringent$effect == '+'),]
Bcell_LAIV_wilcoxon_adult_stringent_upregulation <-
  unique(Bcell_LAIV_wilcoxon_adult_stringent_upregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_adult_stringent_upregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_adult_stringent_upregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_adult_stringent_upregulation <- Bcell_LAIV_wilcoxon_adult_stringent_upregulation %>% arrange(days,desc(log2FC_mean)) 
Bcell_LAIV_wilcoxon_adult_stringent_upregulation <- Bcell_LAIV_wilcoxon_adult_stringent_upregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.25))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_adult_stringent_upregulation),sheetName = 'upregulation',
           'tonsil_LAIV_120a_sBcell_lognorm_LAIV_bootstrap_stringent_adult_vs_both.xlsx')

Bcell_LAIV_wilcoxon_adult_stringent_downregulation <- 
  Bcell_LAIV_wilcoxon_adult_stringent[(Bcell_LAIV_wilcoxon_adult_stringent$significant == 'yes') & 
                                        (Bcell_LAIV_wilcoxon_adult_stringent$effect == '-'),]
Bcell_LAIV_wilcoxon_adult_stringent_downregulation <-
  unique(Bcell_LAIV_wilcoxon_adult_stringent_downregulation[,c('day_group','marker','log2FC_mean','pval_adj_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_adult_stringent_downregulation$days <- 
  substring(Bcell_LAIV_wilcoxon_adult_stringent_downregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_adult_stringent_downregulation <- Bcell_LAIV_wilcoxon_adult_stringent_downregulation %>% arrange(days,log2FC_mean) 
Bcell_LAIV_wilcoxon_adult_stringent_downregulation <- Bcell_LAIV_wilcoxon_adult_stringent_downregulation %>% group_by(day_group) %>% 
  mutate(n_significant = length(unique(marker)),n_distinct = sum(abs(log2FC_mean) >= 0.25))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_adult_stringent_downregulation),sheetName = 'downregulation',
           'tonsil_LAIV_120a_sBcell_lognorm_LAIV_bootstrap_stringent_adult_vs_both.xlsx',append = T)

# for (day_group_name in LAIV_day_group_list) {
#   print(day_group_name)
#   graphics.off()
#   temp <- unique(Bcell_LAIV_wilcoxon_adult_stringent[(Bcell_LAIV_wilcoxon_adult_stringent$day_group == day_group_name),][,c('marker','log2FC_mean','pval_adj_mean','effect','significant','label')])
#   temp$log2_log2FC_mean <- sign(temp$log2FC_mean)*log2(abs(temp$log2FC_mean) + 1)
#   temp$significant[(abs(temp$log2FC_mean) >= 0.25) & (temp$significant == 'yes')] <- 'significant and distinct'
#   temp$significant[(abs(temp$log2FC_mean) < 0.25) & (temp$significant == 'yes')] <- 'significant'
#   plot<-ggplot(temp, aes(x=log2_log2FC_mean, y=-log10(pval_adj_mean),color = significant)) +
...
#     geom_hline(yintercept=c(-log10(p_cutoff)), linetype="dotted")#+
#   print(plot)
#   dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_stringent_adult_vs_both_',day_group_name,'.pdf',sep = ''),width = 6.5, height = 5)
# }
# 
############## venn diagram #########################
...
temp <- read.xlsx('./Bcell_age_LAIV_wilcoxon_bootstrap/tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n316_toddler_vs_adult.xlsx',sheetName = 'downregulation')
...
temp <- read.xlsx('./Bcell_age_LAIV_wilcoxon_bootstrap/tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_bootstrap_n316_toddler_vs_adult.xlsx',sheetName = 'upregulation')
temp11 <- unique(temp1$marker[temp1$day_group != 'day0'])
...
Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation_shrink <- 
  Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation[Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation$marker %in% 
                                                    overlapped_downregulation_marker_list,]
temp <- Bcell_LAIV_wilcoxon_youth_vs_adult_downregulation_shrink[,c('marker','days','log2FC_mean')]
...
####### 
temp <- Bcell_LAIV_wilcoxon[(Bcell_LAIV_wilcoxon$pval_adj <= 0.05) & 
                                              (Bcell_LAIV_wilcoxon$age_group2 == '27-39yrs') & 
                                              (!(is.na(Bcell_LAIV_wilcoxon$pval_adj))),]
temp <- unique(temp[,c('marker','effect','n_significant')])
temp <- temp %>% arrange(effect,desc(n_significant))

############## stringent comparison ##############
# stringent
Bcell_LAIV_wilcoxon_youth_stringent <- Bcell_LAIV_wilcoxon_youth
Bcell_LAIV_wilcoxon_stringent <- Bcell_LAIV_wilcoxon_stringent %>% group_by(day_group,marker,bootstrap_index) %>% 
  mutate(significant = ifelse(all((pval_adj <= 0.05) & (prod(diff) > 0)), 'yes','no'))
Bcell_LAIV_wilcoxon_stringent <- Bcell_LAIV_wilcoxon_stringent %>% group_by(day_group,marker,effect) %>% 
  mutate(n_significant = sum(significant == 'yes')/2)
Bcell_LAIV_wilcoxon_stringent <- Bcell_LAIV_wilcoxon_stringent %>% group_by(day_group,marker,effect) %>% 
  mutate(significant = ifelse(n_significant > floor(n_bootstrap_final*0.95), "yes", "no"))
Bcell_LAIV_wilcoxon_stringent <- Bcell_LAIV_wilcoxon_stringent %>% group_by(day_group,marker) %>% 
  mutate(diff_mean = mean(diff),p_mean = exp(mean(log(pval_adj))))
Bcell_LAIV_wilcoxon_stringent$label <- Bcell_LAIV_wilcoxon_stringent$marker
Bcell_LAIV_wilcoxon_stringent$label[Bcell_LAIV_wilcoxon_stringent$significant == 'no'] <- ''

Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- Bcell_LAIV_wilcoxon_stringent[(Bcell_LAIV_wilcoxon_stringent$significant == 'yes') & 
                                                                                              (Bcell_LAIV_wilcoxon_stringent$effect == '+'),]
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- unique(Bcell_LAIV_wilcoxon_youth_stringent_upregulation[,c('day_group','marker','diff_mean','p_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- unique(Bcell_LAIV_wilcoxon_youth_stringent_upregulation %>% group_by(day_group,marker) %>% mutate(n2 = mean(n2)))
Bcell_LAIV_wilcoxon_youth_stringent_upregulation$days <- substring(Bcell_LAIV_wilcoxon_youth_stringent_upregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- Bcell_LAIV_wilcoxon_youth_stringent_upregulation %>% arrange(days,p_mean) 
Bcell_LAIV_wilcoxon_youth_stringent_upregulation <- Bcell_LAIV_wilcoxon_youth_stringent_upregulation %>% group_by(day_group) %>% mutate(n = length(unique(marker)))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_stringent_upregulation),sheetName = 'upregulation',
           'tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_diff_stringent_toddler_vs_both_older.xlsx')

Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- Bcell_LAIV_wilcoxon_stringent[(Bcell_LAIV_wilcoxon_stringent$significant == 'yes') & 
                                                                                                (Bcell_LAIV_wilcoxon_stringent$effect == '-'),]
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- unique(Bcell_LAIV_wilcoxon_youth_stringent_downregulation[,c('day_group','marker','diff_mean','p_mean','n1','n2','ENTREZID')])
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- unique(Bcell_LAIV_wilcoxon_youth_stringent_downregulation %>% group_by(day_group,marker) %>% mutate(n2 = mean(n2)))
Bcell_LAIV_wilcoxon_youth_stringent_downregulation$days <- substring(Bcell_LAIV_wilcoxon_youth_stringent_downregulation$day_group,first = 4,last = 30)
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- Bcell_LAIV_wilcoxon_youth_stringent_downregulation %>% arrange(days,p_mean) 
Bcell_LAIV_wilcoxon_youth_stringent_downregulation <- Bcell_LAIV_wilcoxon_youth_stringent_downregulation %>% group_by(day_group) %>% mutate(n = length(unique(marker)))
write.xlsx(data.frame(Bcell_LAIV_wilcoxon_youth_stringent_downregulation),sheetName = 'downregulation',
           'tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_diff_stringent_toddler_vs_both_older.xlsx',append = T)

for (day_group_name in LAIV_day_group_list[2:13]) {
  print(day_group_name)
  graphics.off()
  temp <- unique(Bcell_LAIV_wilcoxon_stringent[(Bcell_LAIV_wilcoxon_stringent$day_group == day_group_name),][,c('marker','diff_mean','p_mean','effect','significant','label')])
  ...
  print(plot)
  dev.print(pdf, paste('tonsil_LAIV_120a_s_Bcell_LAIV_wilcoxon_diff_stringent_toddler_vs_both_older_',day_group_name,'.pdf',sep = ''),width = 5.5, height = 5)
